...
(defn set-topic
  ...
  (let [all-chores (sort-by :chore-name (map #(do {:slack-handle (:slack-handle (user/get-next-user (:chore-order %)))
    ...
    (slack/set-topic config/chores-channel (string/join " | " (map #(format "%s: %s" (:chore-name %) (:slack-handle %)) all-chores)))))
  ...
  (admin-wrapper
    (fn [channel [chore-name username] _]
      (if (and chore-name username)
        (if-let [user (user/get-by-username (slack/normalize-username username))]
...
(def add-user (admin-wrapper
                (fn [channel [handle] _]
                  (if-let [handle (slack/normalize-username handle)]
                 ...
                 (fn [channel [chore-name & description] _]
                   (if chore-name
                     (if-not (empty? (keep identity description))
                       (let [description (string/join " " description)]
